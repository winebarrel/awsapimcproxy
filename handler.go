...
// profiles (names, AWS profile and region only, never credentials). It lets a
// client discover the valid values for the injected "profile" argument.
func (proxy *Proxy) listProfilesTool() (*mcp.Tool, mcp.ToolHandler) {
	tool := &mcp.Tool{
		Name:        "list_profiles",
		Description: "List the AWS profiles configured in awsapimcproxy. Use the returned names as the 'profile' argument of the other tools.",
	...
	handler := func(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		for i, profile := range proxy.config.Profiles {
			profiles[i] = profileInfo{Name: profile.Name, AWSProfile: profile.AWSProfile, Region: profile.Region}
		}
...
// injected, together with a handler that forwards the call to the profile's
// upstream AWS API MCP server.
...
// closing the removed session to terminate its subprocess.
...
// closeSessions closes and clears every cached upstream session, terminating
// their subprocesses.
